...
class Asana_Client
{
     ...
     * @return string JSON or null
     */
    public function getTasksByFilter($filter = array('assignee' => '', 'project' => '', 'workspace' => '', 'tag'=>'', 'completed_since'=> '', 'modified_since'=>'','per_page'=>''), array $opts = array())
    {
        $url = '';
        $filter = array_merge(array('assignee' => '', 'project' => '', 'workspace' => '', 'tag'=>'',  'completed_since'=>'', 'modified_since'=>'','per_page'=>''), $filter);
        $url .= $filter['assignee'] !== '' ? '&assignee=' . $filter['assignee'] : '';
        $url .= $filter['project'] !== '' ? '&project=' . $filter['project'] : '';
        $url .= $filter['tag'] !== '' ? '&tag=' . $filter['tag'] : '';
        $url .= $filter['workspace'] !== '' ? '&workspace=' . $filter['workspace'] : '';
        $url .= $filter['completed_since'] !== '' ? '&completed_since=' . $filter['completed_since'] : '';
        $url .= $filter['modified_since'] !== '' ? '&modified_since=' . $filter['modified_since'] : '';
        $url .= $filter['per_page'] !== '' ? '&per_page=' . $filter['per_page'] : '';
        if (count($opts) > 0) {
